 ...
 * 保存接收到的点云数据供后续处理
 */
void CostmapGenerator::onPointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr msg) { 
    points_ = msg; 
}
...
void CostmapGenerator::onTimer() {
    ...
    try {
        transform = tf_buffer_.lookupTransform(costmap_frame_, input_frame_, tf2::TimePointZero, tf2::durationFromSec(1.0));
...
void CostmapGenerator::setGridCenter(const geometry_msgs::msg::TransformStamped &tf) {
    ...
    // 输出调试信息
    RCLCPP_DEBUG(this->get_logger(), "Grid center set to vehicle position: x=%.2f, y=%.2f", 
                grid_position_x_, grid_position_y_);
}
...
void CostmapGenerator::publishCostmap(const GridMap &costmap, const geometry_msgs::msg::TransformStamped &tf) {
    ...
    // 输出yaw值以确保正确性
    RCLCPP_INFO(this->get_logger(), "Vehicle yaw for OccupancyGrid: %.2f radians (%.2f degrees)", 
               yaw, yaw * 180.0 / M_PI);
...
Eigen::MatrixXf CostmapGenerator::generatePointsCostmap(const sensor_msgs::msg::PointCloud2::SharedPtr &in_points,
    ...
    try {
        // 查找从点云坐标系到代价地图坐标系的变换
        transform = tf_buffer_.lookupTransform(costmap_frame_, in_points->header.frame_id, 
                                              in_points->header.stamp, tf2::durationFromSec(1.0));
    } catch (tf2::TransformException &ex) {
    ...
    // 输出点云转换信息
    RCLCPP_INFO(this->get_logger(), "Point cloud transform: tx=%.2f, ty=%.2f, tz=%.2f, qw=%.2f, qx=%.2f, qy=%.2f, qz=%.2f",
    ...
    if (!pcl_pointcloud.empty()) {
        ...
        RCLCPP_INFO(this->get_logger(), 
                   ...
                   (min_x + max_x) / 2.0, (min_x + max_x) / 2.0 - grid_position_x_,
                   (min_y + max_y) / 2.0, (min_y + max_y) / 2.0 - grid_position_y_);
    }

    // 从点云生成代价地图
    Eigen::MatrixXf costmap = points2costmap_.makeCostmapFromPoints(maximum_height_thres_, minimum_height_thres_, 
                                                                grid_min_value_, grid_max_value_, 
                                                                costmap_, LayerName::points, pcl_pointcloud);
    ...
    RCLCPP_INFO(this->get_logger(), "Generated costmap with %d non-zero cells out of %dx%d", 
               non_zero_cells, costmap.rows(), costmap.cols());
