...
private:
    ...
    // 加载边界文件
    bool loadBoundaryFile(const std::string &file_path,
    ...
    // 查找最近点索引
    size_t findClosestPointIndex(const std::vector<BoundaryPoint> &boundary_points, double east,
                                 double north, const std::string &boundary_name = "");

    // 计算单个点的成本（用于优化搜索）
    double calculatePointCost(const std::vector<BoundaryPoint> &boundary_points, size_t index,
                              double cur_yaw_rad, size_t last_closest_idx, bool first_run);

    // 计算指定长度的边界点
    void calculateBoundarySegment(const std::vector<BoundaryPoint> &boundary_points,
