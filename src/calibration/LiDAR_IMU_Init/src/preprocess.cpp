...
void Preprocess::process(const LivoxCustomMsg::ConstSharedPtr &msg, PointCloudXYZI::Ptr &pcl_out) {
    avia_handler(msg);
    *pcl_out = pl_surf;
}

void Preprocess::process_cut_frame_livox(const LivoxCustomMsg::ConstSharedPtr &msg,
    ...
    double last_frame_end_time = time_to_sec(msg->header.stamp) * 1000.0;
    uint valid_num = 0;
    uint cut_num = 0;
    ...
    for (uint i = 1; i < valid_pcl_size; i++) {
        ...
        pl_surf[i].curvature += time_to_sec(msg->header.stamp) * 1000.0 - last_frame_end_time;
...
//            added_pt.curvature = (pl_orig.points[i].timestamp - msg->header.stamp.toSec()) * 1000;  //s to ms
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            added_pt.curvature = (pl_orig.points[i].timestamp - time_to_sec(msg->header.stamp) + 0.1) * 1000.0;  //ms
...
#if LIDAR_IMU_INIT_HAS_LIVOX_CUSTOM_MSG
void Preprocess::avia_handler(const LivoxCustomMsg::ConstSharedPtr &msg) {
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
