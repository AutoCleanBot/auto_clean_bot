...
#ifndef LIDAR_IMU_INIT_HAS_LIVOX_CUSTOM_MSG
#define LIDAR_IMU_INIT_HAS_LIVOX_CUSTOM_MSG 1
#endif
#elif __has_include(<livox_ros_driver/msg/custom_msg.hpp>)
...
#else
#ifndef LIDAR_IMU_INIT_HAS_LIVOX_CUSTOM_MSG
#define LIDAR_IMU_INIT_HAS_LIVOX_CUSTOM_MSG 0
#endif
#endif
...
POINT_CLOUD_REGISTER_POINT_STRUCT(
    ouster_ros::Point, (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)
...
POINT_CLOUD_REGISTER_POINT_STRUCT(robosense_ros::Point, (float, x, x)(float, y, y)(float, z, z)
                                  // use std::uint32_t to avoid conflicting with pcl::uint32_t
                                  (std::uint8_t, intensity, intensity)(std::uint16_t, ring,
...
public:
    ...
    void process(const LivoxCustomMsg::ConstSharedPtr &msg, PointCloudXYZI::Ptr &pcl_out);
    void process_cut_frame_livox(const LivoxCustomMsg::ConstSharedPtr &msg,
    ...
    void process_cut_frame_pcl2(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg,
                                deque<PointCloudXYZI::Ptr> &pcl_out, deque<double> &time_lidar,
                                const int required_frame_num, int scan_count);
