...
void ImuProcess::propagation_and_undist(const MeasureGroup &meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  double imu_end_time = time_to_sec(v_imu.back()->header.stamp);
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    if (time_to_sec(tail->header.stamp) < last_lidar_end_time_)    continue;
    ...
    acc_avr   <<0.5 * (head->linear_acceleration.x + tail->linear_acceleration.x),
      ...
      fout_imu << setw(10) << time_to_sec(head->header.stamp) << "  " << angvel_now.transpose()<< " " << acc_now.transpose() << endl;

    angvel_avr -= state_inout.bias_g;
    acc_avr     = acc_avr / IMU_mean_acc_norm * G_m_s2 - state_inout.bias_a;

    if(time_to_sec(head->header.stamp) < last_lidar_end_time_)
        dt = time_to_sec(tail->header.stamp) - last_lidar_end_time_;
    else
        dt = time_to_sec(tail->header.stamp) - time_to_sec(head->header.stamp);
    ...
    acc_s_last  = acc_imu;
    double &&offs_t = time_to_sec(tail->header.stamp) - pcl_beg_time;
