...
#include "radiolink/gps_serial.h"
#include <chrono>
#include <cstdio>
#include <string>
#include <fstream>
...
namespace radiolink {
RadioLinkNode::RadioLinkNode() : Node("rtk_node") {
    // constructor
    InitValues();
    InitParams();
    bool ret = DeviceInit();
    if (ret) {
        running_ = true;
        read_thread_ = std::shared_ptr<std::thread>(new std::thread(std::bind(&RadioLinkNode::InfoReadLoop, this)));
        read_thread_->detach();
    } else {
    ...
    local_timer_ = this->create_wall_timer(std::chrono::milliseconds(time_interval),
                                           std::bind(&RadioLinkNode::LocalTimerCallback, this));
    time_interval = static_cast<int>(1000.0 / imu_publish_rate_);
    imu_timer_ =
        this->create_wall_timer(std::chrono::milliseconds(time_interval), std::bind(&RadioLinkNode::ImuTimerCallback, this));
    time_interval = static_cast<int>(1000.0 / gnss_pose_enu_publish_rate_);
    gnss_timer_ =
        this->create_wall_timer(std::chrono::milliseconds(time_interval), std::bind(&RadioLinkNode::GNSSTimerCallback, this));
...
void RadioLinkNode::LocalTimerCallback() {
    if (running_ && base_point_set_) {
        pub_localization_info_->publish(rtk_msg_);
    }
}

void RadioLinkNode::ImuTimerCallback() {
    if (running_) {
        pub_imu_->publish(imu_msg_);
    }
}

void RadioLinkNode::GNSSTimerCallback() {
    if (running_ && base_point_set_) {
 ...
 * @param giavp
 */
void RadioLinkNode::WGS84toENU(const Giavp &giavp) {
...
/**
 * @brief parse the radiolink info string
 *
 * @param info_str
 */
void RadioLinkNode::ParseRTKInfo(const std::string &info_str) {
    // 更新解析计数器
    parse_count_++;
    ...
    if (parsed_fields != 23 && enable_debug_log_) {
        RCLCPP_ERROR(this->get_logger(), "parsed_fields:%d,Failed to parse radiolink info: %s", parsed_fields,
                     info_str.c_str());
    }
    ...
    if (should_log) {
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Received radiolink info (Parse #%d): "
    ...
    // simply publish the parsed radiolink info
...
void RadioLinkNode::InfoReadLoop() {
    std::string data;
...
void RadioLinkNode::InitInfoStrSaving() {
...
void RadioLinkNode::SaveInfoStrToFile(const std::string &info_str) {
...
std::string RadioLinkNode::GenerateTimestampFilename() {
...
int main(int argc, char *argv[]) {
    ...
    auto node = std::make_shared<radiolink::RadioLinkNode>();
    RCLCPP_INFO(node->get_logger(), "radiolink node started");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}