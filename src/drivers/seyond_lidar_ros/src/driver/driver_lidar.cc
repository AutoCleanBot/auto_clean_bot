...
DriverLidar::DriverLidar(const LidarConfig& lidar_config) {
  ...
  inno_pc_file_ = lidar_config.inno_pc_file;
...
int32_t DriverLidar::lidar_set_roi(double horz_angle, double vert_angle) {
  int32_t ret = 0;
  if (lidar_handle_ < 0) {
...
int32_t DriverLidar::inno_pc_playback_process() {
  lidar_handle_ =
      inno_lidar_open_file(lidar_name_.c_str(), inno_pc_file_.c_str(), false, packet_rate_, file_rewind_, 0);
  if (lidar_handle_ < 0) {
    inno_log_error("FATAL: Lidar %s invalid handle", lidar_name_.c_str());
    return -1;
  }
